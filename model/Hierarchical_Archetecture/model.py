...
class AttentionWordRNN(nn.Module):
	...
	def __init__(self, batch_size, num_tokens, embed_size, word_gru_hidden, bidirectional= True):        
		...
		if bidirectional == True:
			...
			self.weight_W_word = nn.Parameter(torch.randn(word_gru_hidden, word_gru_hidden))
			self.bias_word = nn.Parameter(torch.randn(word_gru_hidden,1))
			self.weight_proj_word = nn.Parameter(torch.randn(word_gru_hidden, 1))
...
class AttentionSentRNN(nn.Module):
	...
	def __init__(self, batch_size, sent_gru_hidden, word_gru_hidden, n_classes, bidirectional= True):        
		...
		if bidirectional == True:
			...
			self.weight_W_sent = nn.Parameter(torch.randn(sent_gru_hidden ,sent_gru_hidden))
			self.bias_sent = nn.Parameter(torch.randn(sent_gru_hidden,1))
			self.weight_proj_sent = nn.Parameter(torch.randn(sent_gru_hidden, 1))
