...
def training_loop(config, model, loss_, time_loss_, optim, train_data, training_iter, dev_iter, test_iter, logger, savepath):
	...
	while epoch <= config['num_epochs']:
		...
		labels = labels.data.numpy()
		labels_all += list(labels)
		#print('the labels shape is ' + str(len(labels)))
		#print('the labels_all shape is ' + str(len(labels_all)))
		output = F.softmax(output)
		output= output.cpu()
		output_all += list(output.data.numpy())
		#print('the outputs shape is ' + str(len(output)))
		#print('the output_all shape is ' + str(len(output_all)))
	
		if step % total_batches == 0 and step != 0:
		    output_all = np.array(output_all)
			...
			print('This is the output_all')
			print(output_all)
			print('This is the label_all')
			print(labels_all)
