...
class Hierachical_BiGRU_max(nn.Module):
	def __init__(self, config):
		...
		self.word_embed = nn.Embedding(vocab_size + 1, words_dim)
		if self.embed_mode != 'random':
		    print('embedding matrix used!')
		    embedding_mtrx = pickle.load(open(os.path.join(config['DATAPATH'],'embedding_mtrx.p'), "rb" ) )
            self.word_embed.weight.data.copy_(torch.from_numpy(embedding_mtrx))
            if config['embed_tuned'] == False:
